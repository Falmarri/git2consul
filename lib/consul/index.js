...
var write_content_to_consul = function(key_name, content, cb) {
  ...
  ret = {key_name : content};
  consul.kv.set({'key': key_name, value: content, token: token}, function(err) {
    if (err) {
      return cb(ret, 'Failed to write key ' + key_name + ' due to ' + err);
    }
    cb(ret);
  });
};
...
var file_deleted = function(branch, file, cb) {
  ...
  var ret = {'key_name': null};
  consul.kv.del({'key': key_name, token: token}, function(err) {
    /* istanbul ignore if */
    if (err) return cb(ret, 'Failed to delete key ' + key_name + ' due to ' + err);
    cb(ret);
  });
};
...
var process_records = function(branch, records, cb) {
  ...
  var check_pending = function(rec, err) {
    if (err) {
    ...
    if (pending_records === 0) {
      cb(updates, (errors_seen.length > 0) ? errors_seen : null);
    }
  ...
  records.forEach(function(record) {
    ...
    switch (record.type) {
      ...
      case 'T':
        ...
        ++pending_records;
        updates, file_modified(branch, record.path, check_pending);
        break;
      case 'D':
        // Delete file
        ++pending_records;
...
exports.handleRefChange = function(branch, cb) {
  ...
  branch.getCurrentRef(function(err, ref) {
    ...
    var handle_records = function(err, records) {
      /* istanbul ignore if */
      if (err) return cb(err);
      process_records(branch, records, function(kvs, errs) {

        logger.trace({"consul" : kvs}, "Consul Records");
        ...
        exports.setLastProcessedRef(branch, ref, function(n, err) {
          return cb(err);
        });
      });
    };
